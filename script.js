...
function calculateTrueAPI() {
  ...
  const N_spm = parseFloat(document.getElementById('in_N').value) || 0;
  ...
  const Sp_dynamic = Math.max(...dyno_pump.map(p => p.x)) - Math.min(...dyno_pump.map(p => p.x));
  let PD = 0.1166 * Sp_dynamic * N_spm * Math.pow(D, 2);

  // Keep unconverted variables specifically for the drawing engine
  // to guarantee the mathematical shape doesn't warp when UI unit changes
...
// Explicitly decoupled from UI Metric logic to preserve geometrical accuracy
function drawDynamicDyno(surfacePoints, pumpPoints, maxDisp, fo, wrf, isMetric) {
  ...
  // Use raw imperial coordinates internally for perfect scaling
  ...
  // Calculate specific label text depending on UI toggle
  ...
  // Draw Dynamic Downhole Pump Card (Blue)
  ctx.beginPath();
  for(let i = 0; i < pumpPoints.length; i++) {
    let x_val = pumpPoints[i].x;
    let y_val = pumpPoints[i].y + wrf; 
