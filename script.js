...
function exportData() { alert(`Generating Predictive Wave Report for ${currentUser.email}...`); }
...
window.onload = function() {
  ...
  document.getElementById('unit_toggle').addEventListener('click', function() {
    ...
    if (isMetric) {
      ...
      // Plunger & Tubing intentionally left standard API sizes
    } else {
...
// Expanded Math Matrix for accurate weights and tapers for standard API Plunger sizes
const API_ROD_TABLE = {
  "66": { 
...
const API_TUBING_TABLE = { "2.375": 0.307, "2.875": 0.226, "3.500": 0.158, "4.500": 0.096 };
...
// ========================================================================
// 1D DAMPED WAVE EQUATION ENGINE (FINITE-DIFFERENCE NUMERICAL SOLVER)
// ========================================================================
function calculateTrueAPI() {
  ...
  // Plunger and Tubing always read natively in inches directly from Dropdowns
  ...
  // Convert strictly metric inputs back to imperial for accurate physics engine math
  ...
  let Wr = 1.85; let Er_raw = 0.800 * 1e-6; let activeTaper = []; let Et_raw = 0;
  ...
  // --- REVISED OUTPUT CALCULATION LOGIC ---
  let CBE = 1.06 * (Wrf + (0.5 * Fo)); 
  
  let max_torque_up = (pprl_wave - CBE) * (S / 2) * 1.04;
  let max_torque_down = (CBE - mprl_wave) * (S / 2) * 1.04;
  const PT = Math.max(max_torque_up, max_torque_down); // Computed in in-lbs
  
  const PRHP = (pprl_wave - mprl_wave) * S * N_spm * 1.5e-6; 
  
  // Unanchored Tubing Stretch Compensation
  ...
  // Send pure imperial data to drawing engine to prevent visual warping
  ...
  let disp_S = S; let out_Sp = Sp_dynamic; let out_PRHP = PRHP;

  // --- UNIT CONVERSION & UI UPDATES ---
  if (isMetric) {
    PD *= 0.158987; out_PPRL *= 0.453592; out_PT *= 0.011521; out_MPRL *= 0.453592; out_PRHP *= 0.7457; CBE *= 0.453592;
    out_Fo *= 0.453592; out_Wr *= 1.48816; out_Wrf *= 0.453592; out_Sp *= 0.0254; disp_S *= 0.0254;

    safeUpdate('unit_14', "m³/d"); safeUpdate('unit_23', "kg"); 
    safeUpdate('unit_25', "kg-m"); // Torque in kg-m
    ...
    out_PT /= 12; // Convert strictly to lbs-ft for Imperial output
    
    safeUpdate('unit_14', "bpd"); safeUpdate('unit_23', "lbs"); 
    safeUpdate('unit_25', "lbs-ft"); // Output in foot-pounds
  ...
  safeUpdate('out_26', out_PRHP.toFixed(1));
...
// --- DYNAMOMETER PLOTTING ENGINE ---
// Expects input arrays natively generated in raw Imperial units
function drawDynamicDyno(surfacePoints, pumpPoints, maxDisp, fo, wrf, isMetric) {
  ...
  // Draw True QROD Downhole Pump Card (Plots purely from 0 to Fo, independent of rod weight Wrf)
  ctx.beginPath();
