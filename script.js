...
function calculateTrueAPI() {
  ...
  const use_sinker = document.getElementById('in_sinker').value === 'yes'; // NEW SINKER VAR
  ...
  const PT = (pprl_wave - mprl_wave) * (S / 4); 
  ...
  // --- DRAW REAL DYNAMIC DYNAMOMETER ---
  drawDynamicDyno(dyno_surface, dyno_pump, disp_S, out_Fo, out_Wrf, isMetric);

  // ==========================================================
  // SINKER BAR & API TAPER ENGINE (DYNAMICALLY UPDATED)
  // ==========================================================
  ...
  if (use_sinker) {
    ...
    let W_sb_air = 4.16; // Standard weight (lbs/ft) for 1-1/4" sinker bar
    let W_sb_fluid = W_sb_air * buoyant_factor;
    L_sb = Math.ceil((Fo / W_sb_fluid) / 25) * 25; 
